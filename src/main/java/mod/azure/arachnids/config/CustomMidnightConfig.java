...
public abstract class CustomMidnightConfig {
  ...
  private static final Pattern DECIMAL_ONLY =
      Pattern.compile("-?([\\d]+\\.?[\\d]*|[\\d]*\\.?[\\d]+|\\.)");
  private static final Pattern HEXADECIMAL_ONLY =
  ...
  private static final Gson gson =
      new GsonBuilder()
          ...
          .addSerializationExclusionStrategy(
              new HiddenAnnotationExclusionStrategy())
          .setPrettyPrinting()
          .create();
  ...
  public static void init(String modid, Class<?> config) {
    ...
    for (Field field : config.getFields()) {
      ...
      if ((field.isAnnotationPresent(Entry.class) ||
           field.isAnnotationPresent(Comment.class)) &&
  ...
  private static void initClient(String modid, Field field, EntryInfo info) {
    ...
    if (e != null) {
      ...
      if (type == int.class)
        textField(info, Integer::parseInt, INTEGER_ONLY, (int)e.min(),
                  (int)e.max(), true);
      else if (type == float.class)
        textField(info, Float::parseFloat, DECIMAL_ONLY, (float)e.min(),
                  (float)e.max(), false);
      else if (type == double.class)
        textField(info, Double::parseDouble, DECIMAL_ONLY, e.min(), e.max(),
                  false);
      else if (type == String.class || type == List.class) {
        info.max =
            e.max() == Double.MAX_VALUE ? Integer.MAX_VALUE : (int)e.max();
        textField(info, String::length, null, Math.min(e.min(), 0),
                  Math.max(e.max(), 1), true);
      } else if (type == boolean.class) {
        Function<Object, Text> func = value
            -> Text.literal((Boolean)value ? "True" : "False")
                   .formatted((Boolean)value ? Formatting.GREEN
                                             : Formatting.RED);
        info.widget =
            new AbstractMap.SimpleEntry<ButtonWidget.PressAction,
                                        Function<Object, Text>>(button -> {
        ...
        Function<Object, Text> func = value
            -> Text.translatable(modid + ".midnightconfig."
  ...
  private static void textField(EntryInfo info, Function<String, Number> f,
                                Pattern pattern, double min, double max,
                                boolean cast) {
    boolean isNumber = pattern != null;
    info.widget =
        (BiFunction<TextFieldWidget, ButtonWidget, Predicate<String>>)(t, b)
        -> s -> {
      s = s.trim();
      ...
      if (inLimits && info.field.getType() != List.class)
        ...
        ((List<String>)info.value)
            .set(info.index,
                 Arrays
                     .stream(
                         info.tempValue.replace("[", "").replace("]", "").split(
                             ", "))
                     .toList()
                     .get(0));
      }
      ...
      if (info.field.getAnnotation(Entry.class).isColor()) {
        ...
        if (!HEXADECIMAL_ONLY.matcher(s).matches())
          return false;
        try {
          info.colorButton.setMessage(Text.literal("⬛").setStyle(
              Style.EMPTY.withColor(Color.decode(info.tempValue).getRGB())));
  ...
  public static void write(String modid) {
    ...
    try {
      ...
      Files.write(
          path,
          gson.toJson(
                  configClass.get(modid).getDeclaredConstructor().newInstance())
              .getBytes());
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  private static class MidnightConfigScreen extends Screen {
    protected MidnightConfigScreen(Screen parent, String modid) {
      super(Text.translatable("text.autoconfig." + modid + ".title"));
      this.parent = parent;
      this.modid = modid;
      this.translationPrefix = "text.autoconfig." + modid + ".";
    }
    ...
    protected void init() {
      ...
      this.addDrawableChild(
          new ButtonWidget(this.width / 2 - 154, this.height - 28, 150, 20,
      ...
      this.list = new MidnightConfigListWidget(
          this.client, this.width, this.height, 32, this.height - 32, 25);
      if (this.client != null && this.client.world != null)
        this.list.setRenderBackground(false);
      this.addSelectableChild(this.list);
      for (EntryInfo info : entries) {
        if (info.id.equals(modid)) {
          Text name = Objects.requireNonNullElseGet(
              info.name,
              ()
                  -> Text.translatable(translationPrefix +
                                       info.field.getName()));
          ...
          if (info.widget instanceof Map.Entry) {
            Map.Entry<ButtonWidget.PressAction, Function<Object, Text>> widget =
                (Map.Entry<ButtonWidget.PressAction, Function<Object, Text>>)
                    info.widget;
            if (info.field.getType().isEnum())
              widget.setValue(
                  value
                  -> Text.translatable(translationPrefix + "enum." +
                                       info.field.getType().getSimpleName() +
                                       "." + info.value.toString()));
            this.list.addButton(
                List.of(new ButtonWidget(width - 160, 0, 150, 20,
                                         widget.getValue().apply(info.value),
                                         widget.getKey()),
                        resetButton),
                name);
            ...
            TextFieldWidget widget = new TextFieldWidget(
                textRenderer, width - 160, 0, 150, 20, null);
            widget.setMaxLength(info.width);
            if (info.index < ((List<String>)info.value).size())
              widget.setText(
                  (String.valueOf(((List<String>)info.value).get(info.index))));
            else
              widget.setText("");
            Predicate<String> processor =
                ((BiFunction<TextFieldWidget, ButtonWidget, Predicate<String>>)
                     info.widget)
                    .apply(widget, done);
            widget.setTextPredicate(processor);
            resetButton.setWidth(20);
            resetButton.setMessage(Text.literal("R").formatted(Formatting.RED));
            ButtonWidget cycleButton = new ButtonWidget(
                width - 185, 0, 20, 20,
                Text.literal(String.valueOf(info.index))
            ...
            this.list.addButton(List.of(widget, resetButton, cycleButton),
                                name);
            ...
            if (info.field.getAnnotation(Entry.class).isColor()) {
              ...
              resetButton.setMessage(
                  Text.literal("R").formatted(Formatting.RED));
              ButtonWidget colorButton = new ButtonWidget(
                  width - 185, 0, 20, 20, Text.literal("⬛"), (button -> {}));
              try {
                colorButton.setMessage(
                    Text.literal("⬛").setStyle(Style.EMPTY.withColor(
                        Color.decode(info.tempValue).getRGB())));
              } catch (Exception ignored) {
              }
              info.colorButton = colorButton;
              this.list.addButton(List.of(widget, colorButton, resetButton),
                                  name);
            } else
      ...
      for (EntryInfo info : entries) {
        if (info.id.equals(modid)) {
          if (list.getHoveredButton(mouseX, mouseY).isPresent()) {
            ClickableWidget buttonWidget =
                list.getHoveredButton(mouseX, mouseY).get();
            Text text = ButtonEntry.buttonsWithText.get(buttonWidget);
            Text name = Text.translatable(this.translationPrefix +
                                          info.field.getName());
  ...
  public static class MidnightConfigListWidget
    ...
    public MidnightConfigListWidget(MinecraftClient minecraftClient, int i,
    ...
    public Optional<ClickableWidget> getHoveredButton(double mouseX,
                                                      double mouseY) {
      for (ButtonEntry buttonEntry : this.children()) {
        if (!buttonEntry.buttons.isEmpty() &&
            buttonEntry.buttons.get(0).isMouseOver(mouseX, mouseY)) {
  ...
  public static class ButtonEntry extends ElementListWidget.Entry<ButtonEntry> {
    ...
    public static final Map<ClickableWidget, Text> buttonsWithText =
        new HashMap<>();
    ...
    public void render(MatrixStack matrices, int index, int y, int x,
      ...
      if (text != null &&
          (!text.getString().contains("spacer") || !buttons.isEmpty()))
        DrawableHelper.drawTextWithShadow(matrices, textRenderer, text, 12,
                                          y + 5, 0xFFFFFF);
    }
