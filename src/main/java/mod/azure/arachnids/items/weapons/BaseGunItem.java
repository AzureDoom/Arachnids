...
public class BaseGunItem extends Item implements IAnimatable, ISyncable {
  ...
  public <P extends Item & IAnimatable>
  ...
  public void onAnimationSync(int id, int state) {
    if (state == ANIM_OPEN) {
      final AnimationController<?> controller =
          GeckoLibUtil.getControllerForID(this.factory, id, controllerName);
      if (controller.getAnimationState() == AnimationState.Stopped) {
        controller.markNeedsReload();
        controller.setAnimation(
            new AnimationBuilder().addAnimation("firing", false));
      }
    }
  }
  ...
  public MZ90Entity createMZ90(World worldIn, ItemStack stack,
                               LivingEntity shooter) {
    MZ90Entity arrowentity = new MZ90Entity(worldIn, shooter, false);
    return arrowentity;
  }

  public FlareEntity createFlare(World worldIn, ItemStack stack,
                                 LivingEntity shooter) {
  ...
  public void appendTooltip(ItemStack stack, World world, List<Text> tooltip,
    ...
    tooltip.add(
        Text.translatable(
                "Ammo: " + (stack.getMaxDamage() - stack.getDamage() - 1) +
  ...
  public void inventoryTick(ItemStack stack, World world, Entity entity,
    ...
    if (world.isClient) {
      if (((PlayerEntity)entity).getMainHandStack().getItem() instanceof
          BaseGunItem) {
    ...
    if (user.getStackInHand(hand).getItem() instanceof BaseGunItem) {
      while (!user.isCreative() && user.getStackInHand(hand).getDamage() != 0 &&
        ...
        user.getEntityWorld().playSound(
            (PlayerEntity)null, user.getX(), user.getY(), user.getZ(),
  ...
  protected void spawnLightSource(Entity entity, boolean isInWaterBlock) {
    if (lightBlockPos == null) {
      ...
      entity.world.setBlockState(
          lightBlockPos, ArachnidsMod.TICKING_LIGHT_BLOCK.getDefaultState());
  ...
  private boolean checkDistance(BlockPos blockPosA, BlockPos blockPosB,
                                int distance) {
    ...
    for (int x : offsets)
      for (int y : offsets)
        for (int z : offsets) {
          ...
          if (state.isAir() ||
              state.getBlock().equals(ArachnidsMod.TICKING_LIGHT_BLOCK))
            return offsetPos;
        }
