...
public class MAR2Item extends BaseGunItemExtended {
	...
	public MAR2Item() {
		super(new Item.Properties().stacksTo(1).durability((ArachnidsMod.config.MAR2_max_ammo + 1)));
					...
					if (playerentity.getOffhandItem().getItem() == ArachnidsItems.MZ90) {
						...
						if (result != null) {
							if (result.getEntity()instanceof LivingEntity livingEntity)
								livingEntity.hurt(playerentity.damageSources().playerAttack(playerentity), (enchantlevel > 0 ? (ArachnidsMod.config.MAR2_bullet_damage + (enchantlevel * 1.5F + 0.5F)) : ArachnidsMod.config.MAR2_bullet_damage));
						} else {
							projectile = createBullet(worldIn, stack, playerentity, ArachnidsMod.config.MAR2_bullet_damage);
	...
	public void appendHoverText(ItemStack stack, Level world, List<Component> tooltip, TooltipFlag context) {
		...
		tooltip.add(Component.translatable("Damage: " + (enchantlevel > 0 ? (ArachnidsMod.config.MAR2_bullet_damage + (enchantlevel * 1.5F + 0.5F)) : ArachnidsMod.config.MAR2_bullet_damage)).withStyle(ChatFormatting.ITALIC));
	}
